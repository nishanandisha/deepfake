...
> ## Role in the current stack
>
> This project still runs, but it is no longer the primary backend. In the
> stack started by `../ai-voice-detect/scripts/start_all.sh` it serves port
> **8001** and contributes its **visual branch only**, marked advisory; the
> audio verdict comes from `../ai-voice-detect`.
>
> That split is measured, not preferential. Probing this model's visual branch
> with degenerate input:
>
> | visual input | p(fake) |
> |---|---|
> | real fake-clip frames | 0.9929 |
> | all black | 0.0039 |
> | all white | 0.9806 |
> | uniform grey | 0.9817 |
> | random noise | 0.8375–0.9449 |
>
> A black clip scores 0.0039 where a real clip scores 0.0038 — the branch
> barely distinguishes them — while any non-face content reads as confidently
> fake. Its Haar face detector misses on 0–69% of frames even within its own
> training distribution, and on failure silently centre-crops background.
>
> It is still the only branch that detects *visual* manipulation, which the
> audio model cannot see at all, so the router escalates on either branch.
> See [`../ai-voice-detect`](../ai-voice-detect) for the combined design.
...
[`../deepfake-detect-ui`](../deepfake-detect-ui) ("**DeepFake**") is the
review UI over this API. It presents the same numbers as a deepfake / not a
deepfake verdict, and localizes the manipulation to specific timestamps in
each track rather than scoring the clip as a whole.

**Audio-only uploads** (voice notes, `.mp3`/`.wav`, or a container with no
video track) are routed to the acoustic branch alone by
`_infer_audio_only()` in `src/inference/pipeline.py`, rather than being
handed to fusion with a blank video track. `y_hat_acoustic_logit` derives
from `pool_ha(acoustic_encoder(...))` and never reads the visual stream, so
this is that branch's own prediction, not a degraded fused one. Such
responses carry `hasVideo: false`, `yHatVisual: null` and an empty
`visualSaliency`. Note the calibration caveat: the temperature and thresholds
were fitted on the fused logit, so an audio-only verdict is less precisely
calibrated — the UI states this.

The wire format is otherwise unchanged — `serve.py` still returns the three-way
`approve`/`flag`/`block` decision documented below, and the UI maps it
(`block` -> Deepfake, `flag` -> Possibly manipulated, `approve` -> Not a
deepfake) using the `tauLo`/`tauHi` shipped with each response. Nothing in
this repo needed to change for that. See the UI's README for the mapping and
for how tampered regions are derived from `visualSaliency` and
`waveform.suspiciousRegions`.

...
real samples, and the hard constraint during calibration) essentially
undefined.
...
all survive into the subset.

That last point matters even for an audio-only model: a subset of purely
audio-manipulated fakes would train a detector that never sees a clip whose
audio is genuine but whose video was edited -- exactly the case it must call
"real".
"""

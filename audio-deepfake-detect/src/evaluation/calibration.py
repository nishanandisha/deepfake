"""Post-hoc temperature scaling for the frozen acoustic classifier.

T is fit on the CALIBRATION split (never val/train) by minimizing NLL;
c = 1 - sigmoid(logit / T) is the calibrated authenticity score used
downstream by the decision policy (src/evaluation/policy.py).

The parent multimodal project calibrated the fused audio-visual logit. This
package calibrates the audio branch on its own, so the thresholds selected
here are valid for an audio-only decision and must NOT be swapped with the
fusion policy: the two models produce differently-scaled logits.
"""
...
def get_acoustic_logits(
    ...
    """Runs the frozen acoustic classifier over `dataset` and returns (raw
    logits, integer labels) in split order -- pre-sigmoid, since temperature
    scaling operates on logits, not probabilities.

    `dataset` is an AcousticDataset, whose batches are
    (features, padding_mask, label)."""
    loader = DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers)
    model.eval()

    logits, labels = [], []
    with torch.no_grad():
        for features, padding_mask, batch_labels in loader:
            outputs = model(features.to(device), padding_mask=padding_mask.to(device))

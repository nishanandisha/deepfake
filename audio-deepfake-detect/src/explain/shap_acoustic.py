"""SHAP attribution over the named acoustic features.

KernelSHAP over the ~68-dim per-frame acoustic descriptor vector (MFCCs +
deltas + delta-deltas, F0, voicing confidence, spectral stats, ZCR,
...
per-frame -- "F0 drove this decision", not "F0 at frame 137".

This is exactly why the encoder keeps hand-crafted named features instead
of a learned embedding: every dimension here maps to a human-readable name
from `feature_names` (src/models/acoustic/features.py).
"""
...
def explain_acoustic_features(
    ...
    """Attributes the acoustic classifier's logit across named descriptors.

    model: an `AcousticClassifier` -- called as model(features, padding_mask)
    and expected to return logits [B].
    acoustic_features: [B, S, D] for the samples being explained.
    background_features: [S, D] reference values (typically the per-column
    mean over a background set, or zeros -- which, after the encoder's input
    BatchNorm, *is* the per-feature mean); a "missing" descriptor is replaced
    by its background column, the standard KernelSHAP absence treatment.
    ...
    def coalition_predict(masks: np.ndarray) -> np.ndarray:
        ...
        with torch.no_grad():
            logits = model(mixed, padding_mask=mask_batch)
...
def top_k_features(attributions: Dict[str, float], k: int = 5) -> List[tuple]:
...
def compute_background_features(background_loader, max_batches: int = 4) -> torch.Tensor:
    ...
    the KernelSHAP reference. Consumes an AcousticDataset loader, whose
    batches are (features, padding_mask, label)."""
    total, count = None, 0
    for batch_idx, (features, _, _) in enumerate(background_loader):

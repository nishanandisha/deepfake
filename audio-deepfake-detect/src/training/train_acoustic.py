"""Acoustic training loop.

`train_acoustic(cfg)` is the reusable entry point -- called by
scripts/train.py via Hydra, and directly by tests with a plain config
object.
"""
...
def train_acoustic(cfg, logger: ExperimentLogger = None) -> Dict[str, float]:
    ...
    # feature_names order must match the tensor column order, since SHAP
    # attributes the decision back to these names. Checked here, before any

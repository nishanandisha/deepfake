"""Building blocks for the acoustic encoder: sinusoidal positional
encoding and mask-aware temporal pooling.

Kept as a separate module rather than folded into encoder.py because these
are modality-agnostic -- in the parent multimodal project the same pooling
serves the visual and fusion encoders too."""
...
class AttentionPool(nn.Module):
    ...
    def attention_weights(self, x: torch.Tensor, padding_mask: torch.Tensor = None):
        """Per-timestep weights, exposed so an explanation can report
        *which* frames the model actually attended to."""

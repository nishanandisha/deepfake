...
embedding): `_feature_names` gives one name per output dimension, in the
exact order features are concatenated, so the SHAP explanations in
src/explain/shap_acoustic.py can attribute the decision back to specific
descriptors (e.g. "F0", "spectral flatness").
"""
...
def extract_acoustic_features(
    ...
    feature_names[i] naming column i of features -- this ordering is what
    makes the SHAP attributions meaningful, and is asserted below.

"""Index the raw dataset and write identity-disjoint split manifests to
data/splits/*.csv.

Identity-disjointness is the point: no identity may appear in two splits,
or the model is scored on speakers it trained on.
"""
...
def main(cfg: DictConfig) -> None:
    ...
    # DFDC, when present, is indexed but never split -- it stays fully held
    # out for cross-dataset evaluation.
    dfdc_root = cfg.data.get("dfdc_root_dir")
    if dfdc_root and Path(dfdc_root).is_dir():
        dfdc_manifest = index_dfdc(dfdc_root)
        dfdc_out_path = splits_dir / "dfdc_holdout.csv"
        dfdc_manifest.to_csv(dfdc_out_path, index=False)
        logger.info(
            f"Wrote {len(dfdc_manifest)} DFDC samples to {dfdc_out_path} "

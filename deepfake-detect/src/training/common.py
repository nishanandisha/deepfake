"""Training engine shared by every standalone branch (visual, acoustic) and,
from Stage 5 on, the joint fusion trainer: device resolution, the
train/eval epoch loop, and the cosine-with-warmup LR schedule. Every branch
dataset yields (inputs, padding_mask, label) batches and every branch model
is called as model(inputs, padding_mask=padding_mask), so this loop is
model-agnostic.
"""
...
def make_pos_weight(cfg, train_labels, device: torch.device) -> torch.Tensor:
    ...
    the extra pos_weight of 0.344 pushed the visual branch into predicting
...
def run_standalone_training(
    ...
    """Shared standalone-branch training loop: sampler/loader setup,
    class-weighted BCE, AdamW + cosine-warmup schedule, early stopping on
    val AUC, checkpointing, and results-markdown writing. Used identically
    by train_visual.py (Stage 2) and train_acoustic.py (Stage 3).
    """

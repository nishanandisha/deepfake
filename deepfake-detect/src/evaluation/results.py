"""Shared results-markdown writer so every branch's standalone training run
produces a consistently formatted row for the Stage 8 baseline comparison
table. Also writes a sibling results.json with the raw metrics dict, so
later stages (5's late-fusion comparison, 8's table assembly) can load
numbers programmatically instead of parsing markdown."""

import json
from pathlib import Path
from typing import Dict, Optional

# Stage 2/3 of the build plan say to stop and debug if AUC is "near 0.5".
# 0.55 was too lenient: the real visual branch scored AUC 0.599 while
# predicting a single class for every clip, and still printed "OK". An AUC
# in the 0.6s on a binary task is barely-above-chance ranking, not a
# working detector, so the bar is 0.65 and degenerate predictors are now
# called out separately regardless of AUC.
...
def write_results_markdown(
    ...
    if auc != auc:  # NaN check without importing math/numpy here
        ...
        status = (
            ...
            "STOP AND DEBUG DATA/LABELS BEFORE PROCEEDING, per Stage 2/3 exit criteria."
        )
    else:

"""One-time preprocessing cache.

Without this, every epoch re-runs Haar-cascade face detection on every
frame and `librosa.pyin` on every audio clip. pyin alone costs ~1-3s per
clip, so a single epoch over a few thousand clips takes hours and a real
training run becomes impossible -- the cost is paid once per epoch when it
should be paid once, ever.

Cached artefacts per clip:
  <cache_dir>/visual/<hash>.npz   aligned face frames, uint8 [T,H,W,3]
  <cache_dir>/acoustic/<hash>.npz named feature matrix, float32 [S,D]

The cache key includes every parameter that changes the output (frame
rate/size/count, sample rate, framing, n_mfcc), so changing a config value
produces a different key rather than silently serving stale tensors.
"""
...
class PreprocessingCache:
    def __init__(self, cache_dir: str, enabled: bool = True):
        ...
        if self.enabled:
            (self.cache_dir / "visual").mkdir(parents=True, exist_ok=True)
            (self.cache_dir / "acoustic").mkdir(parents=True, exist_ok=True)

    # -- visual -----------------------------------------------------------

    def visual_path(self, video_path: str, params: dict) -> Path:
        return self.cache_dir / "visual" / f"{_cache_key(video_path, params)}.npz"

    def get_visual(self, video_path: str, frame_rate: float, frame_size: int) -> np.ndarray:
        """Aligned face frames [T, H, W, 3] uint8, from cache when present."""
        params = {"kind": "visual", "frame_rate": frame_rate, "frame_size": frame_size}

        if self.enabled:
            path = self.visual_path(video_path, params)
            if path.exists():
                try:
        ...
        frames = preprocess_video(video_path, frame_rate=frame_rate, size=frame_size)

        if self.enabled:
            self._atomic_savez(self.visual_path(video_path, params), frames=frames)
        return frames
    ...
    def stats(self) -> dict:
        ...
        visual_files = list((self.cache_dir / "visual").glob("*.npz"))
        acoustic_files = list((self.cache_dir / "acoustic").glob("*.npz"))
        total_bytes = sum(f.stat().st_size for f in visual_files + acoustic_files)
        return {
            "enabled": True,
            "cache_dir": str(self.cache_dir),
            "visual_entries": len(visual_files),
...
def warm_cache(
    ...
    backend that failed 100% of audio decodes, leaving an empty acoustic
    cache that would only have surfaced as a broken training run. So if the
    first `fail_fast_after` clips all fail, stop and raise.
    """
    failures = []
    for i, row in enumerate(manifest.itertuples(index=False)):
        try:
            cache.get_visual(row.video_path, frame_rate, frame_size)
            cache.get_acoustic(

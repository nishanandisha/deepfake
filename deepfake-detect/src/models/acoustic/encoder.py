"""Acoustic branch: linear projection of named per-frame features +
sinusoidal positional encoding + Transformer encoder, per Stage 3 of the
build plan.

AcousticEncoder produces Ha [B, S, d] (reused by Stage 5 fusion).
AcousticClassifier wraps it with the standalone pooling + MLP head used for
this branch's own auxiliary loss.
"""
...
class AcousticEncoder(nn.Module):
    def __init__(
        ...
        # without needing a separate precomputed-statistics artefact. The
        # visual branch gets the equivalent treatment from ImageNet

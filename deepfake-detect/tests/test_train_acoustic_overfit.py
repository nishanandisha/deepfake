"""End-to-end sanity check for the Stage 3 training loop, mirroring
tests/test_train_visual_overfit.py: trains on a tiny synthetic dataset with
a trivially learnable signal (quiet low-frequency "real" tones vs. loud
high-frequency "fake" tones) and asserts the model learns to discriminate.
Confirms training mechanics only -- Stage 3's real exit criteria (AUC
clearly above chance on real FakeAVCeleb val audio) still needs real data.
"""
